...
Una aplicacion de lista de tareas (To-Do List) construida con **React 19**, **TypeScript**, **Zustand** y **Vite**, con una estetica **"Café Notebook"** — cálida, acogedora y moderna.
...
|-- header-divider (linea decorativa)
|-- form-card
|   -- Textbox (formulario)
|-- acciones-masivas (aparece solo si hay seleccionados)
+-- tabla-wrapper
    +-- table.tabla-tareas
       |-- thead (checkbox seleccionar todos + encabezados + filtros)
        +-- tbody (Tarjeta x N)
```
...
### Estetica "Café Notebook"

El diseño sigue la estetica **"Café Notebook"** — cálida y acogedora como un cuaderno de cuero en una cafetería. Se aplican:

- **Tipografía:** `Playfair Display` (serif italica) para el titulo, `Nunito` (redondeada) para el cuerpo.
- **Textura de grano:** capa sutil via CSS SVG sobre toda la pantalla para dar profundidad papelera.
- **Gradiente ambiental:** destello calido naranja/verde en el fondo con `radial-gradient`.
- **Animaciones escalonadas:** las filas de la tabla aparecen con `fadeSlideIn` con delays de 35ms por fila.
- **Tarjeta de formulario:** el formulario flota sobre el fondo con sombra suave y bordes redondeados.

### Paleta de colores

| Variable | Color | Hex | Uso |
|---|---|---|---|
| --espresso | Espresso | #2E1C0E | Encabezado de tabla, texto principal |
| --caramel | Caramel | #C17F3E | Acento principal — boton agregar, focus, checkboxes |
| --parchment | Parchment | #FAF5EE | Fondo calido en modo claro |
| --sage | Sage | #5A7A58 | Exito, badge completado |
| --terracotta | Terracotta | #A8453F | Peligro, badge pendiente, eliminar |
...
:root {
  --bg: #FAF5EE;       /* pergamino calido */
  --text: #6B5744;     /* marron calido */
}

/* Modo oscuro */
:root[data-theme="dark"] {
  --bg: #1A120A;       /* espresso profundo */
  --text: #C4A882;     /* crema calida */
}
```
